...
# and `pip install sfsa_rag_assistant` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "sfsa_rag_assistant", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n sfsa_rag_assistant -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "sfsa_rag_assistant", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "sfsa_rag_assistant", "YOUR_CONSOLE_SCRIPT"]
